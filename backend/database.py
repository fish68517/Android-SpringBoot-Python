...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        for seller_id, title, author, isbn, price, condition, description, status in books_data:
            cursor.execute('''
                INSERT INTO books (seller_id, title, author, isbn, price, condition, description, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (seller_id, title, author, isbn, price, condition, description, status))
